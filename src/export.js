...
const PARAMETER_TEXT_SIZE = 10;
const PARAMETER_TEXT_SPACING = 12;

/**
 * Generates the parameter information text lines for display in exports.
 */
function generateParameterInfo() {
  const has1URows = state.rowIs1U.some((is1U) => is1U);
  const oneUFormat = has1URows ? oneUFormats[state.selected1UFormat].name : 'None';
  const rowAnglesStr = state.rowAngles.join('°, ') + '°';
  
  const lines = [
    `Case Width: ${state.caseWidthHP}HP (${Math.round(state.caseWidth * 100) / 100}mm)`,
    `Module Depth Max: ${state.actualPanelDepth}mm`,
    `Back Wall Depth: ${state.actualPanelBackDepth}mm`,
    `Row Count: ${state.rowCount}`,
    `1U Rows: ${has1URows ? state.rowIs1U.map((is1U, i) => is1U ? (i + 1) : null).filter(x => x !== null).join(', ') : 'None'}`,
    `1U Format: ${oneUFormat}`,
    `Row Angles: ${rowAnglesStr}`,
    `Material Thickness: ${state.caseMaterialThickness}mm`
  ];
...
/**
 * Generates the export filename based on current parameters.
 */
function generateExportFilename(extension) {
  ...
  const caseWidth = `${state.caseWidthHP}hp`;
  const rowAngles = state.rowAngles.map(angle => `${angle}deg`).join('-');
  const moduleDepth = `${state.actualPanelDepth}mm`;
  const moduleBackDepth = `${state.actualPanelBackDepth}mm`;
  const materialThickness = `${state.caseMaterialThickness}mm`;
  
  return `diyEuroCase_${dateStr}_${caseWidth}-${rowAngles}-${moduleDepth}-${moduleBackDepth}-${materialThickness}.${extension}`;
}
...
export function generateSVG() {
  ...
  const padding = 10;
  const textAreaHeight = 120; // Extra space for parameter text

  const layout = buildExportLayout(geometry);
  const parameterInfo = generateParameterInfo();

  const width = layout.totalWidth + padding * 2;
  const height = layout.totalHeight + padding * 2 + textAreaHeight;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}mm" height="${height}mm" viewBox="0 0 ${width} ${height}">
  ...
  // Add parameter information text at top-left
  svg += `
  
  <!-- Parameter Information -->
  <g id="parameter-info" fill="#000000" font-family="Arial, sans-serif" font-size="${PARAMETER_TEXT_SIZE}">`;
  
  parameterInfo.forEach((line, index) => {
    const textY = padding + PARAMETER_TEXT_SPACING + (index * PARAMETER_TEXT_SPACING);
    svg += `
    <text x="${padding}" y="${textY}">${line}</text>`;
  });
  ...
  <g id="${id}" stroke="#FF0000" stroke-width="${SVG_STROKE_WIDTH}" fill="none">
    ...
    const oy = panel.y + padding + textAreaHeight; // Offset by text area height
...
export function generateDXF() {
  ...
  dxf.addLayer("PARAMETER_TEXT", DxfWriter.ACI.GREEN, "CONTINUOUS");
  ...
  dxf.setActiveLayer("PARAMETER_TEXT");
  parameterInfo.forEach((line, index) => {
    const textY = -(PARAMETER_TEXT_SPACING + (index * PARAMETER_TEXT_SPACING)); // DXF Y coordinates are inverted
    dxf.drawText(0, textY, 0, line, PARAMETER_TEXT_SIZE);
  });
  ...
  for (const panel of layout.panels) {
    ...
    const points = panel.points;
    const ox = panel.x;
    const oy = panel.y + textAreaHeight; // Offset by text area height
...
export function downloadDXF() {
  ...
  const filename = generateExportFilename('dxf');
